...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    x = np.linspace(border, img.shape[1] - border, nPointsX, dtype=int)
    y = np.linspace(border, img.shape[0] - border, nPointsY, dtype=int)
    x_grid, y_grid = np.meshgrid(x, y)
    x_grid = x_grid.flatten()
    y_grid = y_grid.flatten()
    for i in range(len(x_grid)):
        vPoints.append([x_grid[i], y_grid[i]])
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # TODO
                # compute the angles
                grad_x_cell = grad_x[start_x:end_x, start_y:end_y]
                grad_y_cell = grad_y[start_x:end_x, start_y:end_y]
                angle = np.arctan2(grad_x_cell, grad_y_cell)
                # compute the histogram
                desc.append(np.histogram(angle, bins=nBins)[0])
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # ! TODO
        vFeature = descriptors_hog(img, grid_points(img, nPointsX, nPointsY, border), cellWidth, cellHeight)
    ...
    print('number of extracted features: ', vFeatures.shape)
...
def bow_histogram(vFeatures, vCenters):
    ...
    # todo
    indx, _ = findnn(vFeatures, vCenters)
    histo, _ = np.histogram(indx, len(vCenters))
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # todo
        vFeatures = descriptors_hog(img, grid_points(img, nPointsX, nPointsY, border), cellWidth, cellHeight)
        histogram = bow_histogram(vFeatures, vCenters)
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    """
    DistPos, DistNeg = float("inf"), float("inf")
    # Find the nearest neighbor in the positive and negative sets and decide based on this neighbor
    # todo
    dists = np.subtract(vBoWPos, histogram)
    dists = np.linalg.norm(dists, axis=1)
    DistPos = dists[np.argmin(dists)]
    
    dists = np.subtract(vBoWNeg, histogram)
    dists = np.linalg.norm(dists, axis=1)
    DistNeg = dists[np.argmin(dists)]
